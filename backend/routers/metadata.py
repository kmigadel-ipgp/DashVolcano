...
async def get_tectonic_settings(db: Database = Depends(get_database)):
    ...
    """
    volcano_settings = db.volcanoes.distinct("tectonic_setting")
    sample_settings = db.samples.distinct("tectonic_setting")
    # Combine and deduplicate
    all_settings = set(volcano_settings) | set(sample_settings)
    return {
...
async def get_tectonic_settings(db: Database = Depends(get_database)):
    ...
    volcano_settings = db.volcanoes.distinct("tectonic_setting")
    # Combine and deduplicate
    all_settings = set(volcano_settings)
    return {
        "count": len(all_settings),
        "data": sorted([s for s in all_settings if s])
    }
...
async def get_rock_types(db: Database = Depends(get_database)):
    """
    Get list of all rock types
    """
    rock_types = db.samples.distinct("rock_type")
    return {
